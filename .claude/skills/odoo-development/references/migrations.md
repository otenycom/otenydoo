...
When creating a new migration **on `dev`**, set the folder name to the module's current manifest version (or one higher). The `merge-branches` tool (`python -m riverdeploy merge-branches`) automatically detects and renumbers migration folders on feature branches that have become stale relative to `max(dev, main)`. See [Deploy Tool — Feature Branch Merges](../../../../../radar/.claude/skills/crewradar-development/references/deploy-tool.md#feature-branch-merges).
...
1. **The auto-bump catch-up (2026-07).** Barney authored its MFNL activation as `19.0.4.26`; dev's auto-bumps independently reached `19.0.4.26`, so a prod restore + `-u` skipped the folder and the workflow never activated. Renumbering to `19.0.5.x` fixed it — and motivated the higher-minor rule above.
